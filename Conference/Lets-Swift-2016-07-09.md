...
## 1.1. 전반적인 내용
**기사 하나 보여줌, TIOBE Index 코드 생산성의 순위, 스택오버플로우에서도 스위프트가 많이 사랑받고 기존 오브젝티브 시는 미래가 밝지 않다, 스위프트 1.2 2015년 중반 에스케이 플레닛 아이오 에스 개발자 2명 5덕스 개발 함. 스위프트 2.2 2016년 초중반 야놀자 아이오세스 개발자 3명, 스위프트를 하겠다, 에피소드 설명, 일정이 가장 힘듬, 새로운걸 시도하기 쉽지 않다. / 스위프트 프로젝트 할까? / 어쨌든 스위프트 개발 해라. / 병목현상 모든 변수는 var 로 통일 , 익숙한  [];ns@ / 옵셔널 그리고 바인딩 컬렉션 타입 프로퍼티 등.. / 바인딩 바인딩이라는 개념 (가드 렛) / 결론 : 해봐야한다. 사용은 거의 유사하다. 단점 : 유지보수가 많이 든다. 하지만 코드 간결성이 좋아. 해결
### 1.1.1. 상세내용 정리
	1. 간결하다.
	2. 별도의 도구없이 작성가능하다.
	3. 다양한 형태로 변환이 가능하다.
...
# 4. 스위프트 Internals
## 1.1. 전반적인 내용
** 스위프트 내부 동작, 스위프트 컴파일러, 스위프트 타입 시스템, 스위프트 타입 인터널, 스위프트 파운데이션, 요약 / LLVM 에 대한 간단한 설명 / 현재는 엑스코드는 LLVM 컴파일러만 제공 / 스위프트 컴파일러는 처음에 2단계가 더 추가되었다. 타입검사를 한다 타입 추적이 안되면 멈춤. 타입이 명시적으로 되어있지 않으면 먹통, 프론트단을 넘어가지 못해서 나타남 SIL 최적화 / 스위프트 타입 시스템, 양방향 타입 추정 가능 타입을 위한 제약 사항으로 되어있다. / hindley-milner algorithm w var let apps abs 알고리즘이 나와있는데 참 신기하다. / 타입 검사기 (type checker - constraints) / Constraint Solving / 타입 추정이 이렇게 과정이 복잡함 / 타입의 내부 구조들, 네이티브 클래스 타입, 열거형 타입 (내부에서는 스위치문 처럼 만들어짐) / 구조체는 대부분의 스텐다드 타입은 90%가 struct로 만들어져있음. 기본이 되는 타입이다. 객체처럼 생명주기를 가지고 있다. / 스위프트 3 파운데이션 / 요약 스위프트 컴파일러 엘엘브이엠 기반으로 만들었고, 테일러 스위프트르 좋아해서 붙었다고?, 타입 시스템은 에이치 엠 시스템을 확장 더블유 알고리즘을 이용해서 제약을 처리, / 영어문서를 10번 이상 읽어봄.  
...
## 1.1. 전반적인 내용
**스위프트를 사용하면서 자바에 비해 좋았던 부분, VIPER 아키텍처 적용 사례 / Stored Property, Computed Property
/ Closure, Function / Functional 통계 계산 로직에서 적극적으로 사용, 특정 날짜의 데이터를 주로 다룸, 스레드만 잘 처리해주면 사용자 체감상 문제 없음 / 프로토콜, 익스텐션 / VIPER 디스트리뷰션, 테스팅, 사용하기 쉬움 Code Generator Rxswift, KVO / 
### 1.1.1. 상세내용 정리
	1. 간결하다.
	2. 별도의 도구없이 작성가능하다.
	3. 다양한 형태로 변환이 가능하다.
...
# 6. RxSwift 시작하기
## 1.1. 전반적인 내용
**1부터 10까지 더하는 문제 1부터 10까지 더하는 문제, 상태를 저장하지 않음, 데이터 플로우, 누군가 자기를 구독해야 이벤트를 발생한다. 우선 만져보자 그리고 이해 안되는 문제들을 찾아서 공부 / RxMarble 사이트 / 플레이 그라운드에서 찾아보세요 / 
### 1.1.1. 상세내용 정리
	1. 간결하다.
	2. 별도의 도구없이 작성가능하다.
	3. 다양한 형태로 변환이 가능하다.
...
# 7. 스위프트 패키지 매니저
## 1.1. 전반적인 내용
**들어가기, 용어 정리, 패키지 매니저 준비, 간단한 프로그램 빌드, 의존성을 포함한 프로그램 빌드, 엑스코드 프로젝트? / 패키지 매니저에 대한 간단한 용어 정리, 스위프트 패키지 2017관점, 홈브루 개발자가 2015년에 애플에서 일하고 있다. 스위프트 패키지 매니저를 설치해서 사용해볼 수 있다. 매니페스트 / 스위프트 언어를 배우면 패키지 매니저를 설정 할 수 있다. 프로젝트 폴더 구조. / 데모 / 의존성을 가지는 프로젝트  / Coders high  / 개인적인 의견 : 배울때 욕심을 부리지 않는다 체력이 부족함.  / 네트워크를 리엑티브로 변경하여 사용 ./RxSwift Community 커뮤니티, RxAlamofire 있음. 모바일 응용프로그램 아키텍쳐, RxTodo예제 4번째를 봐라 (레퍼런스) 
### 1.1.1. 상세내용 정리
	1. 간결하다.
	2. 별도의 도구없이 작성가능하다.
	3. 다양한 형태로 변환이 가능하다.
